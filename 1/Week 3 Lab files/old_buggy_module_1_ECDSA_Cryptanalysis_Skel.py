...
#    raise NotImplementedError()
    ...
    if algorithm == "ecdsa" and givenbits == "msbs": #checked
        s_inv = mod_inv(s, q)
        t = r * s_inv % q
        z = h * s_inv % q
        a = MSB_to_Padded_Int(N, L, list_k_MSB) #abuse of notation (in contrast to modle pdf
        u = (a - z) % q #TODO check for modulo q
        return (t, u)
    elif algorithm == "ecdsa" and givenbits == "lsbs": #checked
        ...
        return (t, u)
    elif algorithm == "ecschnorr" and givenbits == "msbs": #checked
        # In the case of EC-Schnorr, r may be set to h
        ...
        t = h
        return (t, u)
    elif algorithm == "ecschnorr" and givenbits == "lsbs": #checked
        ...
        RuntimeError("setup_hnp_single_sample: Invalid choice of algorithm and/or givenbits")
#    if algorithm == "ecdsa":
...
def setup_hnp_all_samples(N, L, num_Samples, listoflists_k_MSB, list_h, list_r, list_s, q, givenbits="msbs", algorithm="ecdsa"): #checked
...
#    raise NotImplementedError()
    #TODO sanity checks on input values e.g. list length == num_Samples - need to do that?
    t_list = []
    u_list = []
...
#    raise NotImplementedError()
    ...
    return (base_matrix_B, cvp_target_vector_u) #(list of lists, list) #module pdf descriptions; B and u are now scaled
...
def solve_cvp(cvp_basis_B, cvp_list_u): #TODO DEBUG HERE
...
#    raise NotImplementedError()
    ...
    LLL.reduction(fpylll_cvp_basis_B, method='fast')
    ...
    cvp_solution_v = list(CVP.closest_vector(fpylll_cvp_basis_B, cvp_list_u, method='fast')) #list_u of type list is okay
...
def solve_svp(svp_basis_B):
    ...
    return shortest_vector_candidates
    #raise NotImplementedError()
...
#    v_List = solve_cvp(cvp_basis_B, cvp_list_u) #have to use this (instead of list of lists)
##    raise NotImplementedError()
